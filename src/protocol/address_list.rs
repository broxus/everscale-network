...
pub struct BoxedAddressListView<'a>(pub AddressListView<'a>);

impl WriteToPacket for BoxedAddressListView<'_> {
        ...
        T: Write,
    {
        match self {
            AddressView::Udp { ip, port } => {
                0x670da6e7u32.write_to(packet)?;
                ip.write_to(packet)?;
                port.write_to(packet)
            }
            AddressView::Udp6 { ip, port } => {
                0xe31d63fau32.write_to(packet)?;
                ip.write_to(packet)?;
                port.write_to(packet)
            }
            AddressView::Tunnel { to, pubkey } => {
                0x092b02ebu32.write_to(packet)?;
                to.write_to(packet)?;
